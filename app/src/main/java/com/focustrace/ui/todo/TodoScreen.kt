...
fun TodoScreen(viewModel: TodoViewModel, onReport: (Long) -> Unit, onStarted: () -> Unit) {
    ...
    Scaffold(contentWindowInsets = WindowInsets(0, 0, 0, 0),
        floatingActionButton = {
            FloatingActionButton(onClick = { if (!busy) { editingId = null; editorOpen = true } },
                containerColor = MaterialTheme.colorScheme.primary,
                contentColor = MaterialTheme.colorScheme.onPrimary) {
                Icon(Icons.Outlined.Add, contentDescription = "创建待办")
            }
        }) { padding ->
        LazyColumn(Modifier.fillMaxSize().padding(padding).padding(bottom = 80.dp).testTag("todo-list"),
                ...
                is LoadState.Ready -> {
                    ...
                    data.reportId?.let { id ->
                        item(key = "report", contentType = "summary") {
                            TextButton(onClick = { onReport(id) },
                                colors = ButtonDefaults.textButtonColors(contentColor = MaterialTheme.colorScheme.onSurfaceVariant)) {
                                Text("查看专注报告")
                            }
                        }
                    }
                    item(key = "count", contentType = "section") {
                        val completed = remember(data.tasks) { data.tasks.count { it.completed } }
                        Text("待完成 ${data.tasks.size - completed} · 已完成 $completed",
                            style = MaterialTheme.typography.labelLarge, color = MaterialTheme.colorScheme.onSurfaceVariant)
                    }
                    if (data.tasks.isEmpty()) item(key = "empty") {
                        InfoCard("给重要的事留一点时间", "点击右下角 +，创建今天的第一个待办。")
                    }
                    items(data.tasks, key = { it.id }, contentType = { "task" }) { task ->
    ...
    if (data != null) {
        ...
        data.tasks.firstOrNull { it.id == deletingId }?.let { task ->
            AlertDialog(onDismissRequest = { if (!busy) deletingId = null }, title = { Text("删除待办？") },
                ...
                confirmButton = { TextButton(enabled = !busy, onClick = { viewModel.delete(task) { deletingId = null; editorOpen = false; editingId = null } },
    ...
    Card(Modifier.fillMaxWidth().clickable(enabled = !busy, onClickLabel = "编辑待办") { onEdit(task.id) },
        ...
        Row(Modifier.padding(horizontal = 8.dp, vertical = 16.dp), verticalAlignment = Alignment.CenterVertically,
            ...
            Column(Modifier.weight(1f), verticalArrangement = Arrangement.spacedBy(4.dp)) {
                ...
                Text(if (task.timerType == 1) "正向计时" else "番茄钟 · ${task.targetMinutes} 分钟",
                    style = MaterialTheme.typography.bodySmall, color = MaterialTheme.colorScheme.onSurfaceVariant)
                if (task.completed) Text("已完成", style = MaterialTheme.typography.labelMedium, color = MaterialTheme.colorScheme.onSurfaceVariant)
            }
            if (!task.completed) FilledIconButton(onClick = { onStart(task.id) }, enabled = !busy,
                modifier = Modifier.size(48.dp).testTag("start-task-${task.id}")) {
