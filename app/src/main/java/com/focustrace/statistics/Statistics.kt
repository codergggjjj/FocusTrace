...
data class StatisticsSummary(val range: StatisticsRange, val totals: StatisticsTotals,
    val days: List<DailyStatistics>, val categories: List<CategoryStatistics>, val sessions: List<com.focustrace.data.local.entity.FocusSessionEntity>, val hours: List<StatisticsTotals> = emptyList())
...
fun summarizeStatistics(records: List<StatisticsRecord>, range: StatisticsRange): StatisticsSummary {
    ...
    return StatisticsSummary(range, totals(eligible), days, categories, eligible.map { it.session }.sortedWith(compareByDescending<com.focustrace.data.local.entity.FocusSessionEntity> { it.startTime }.thenByDescending { it.id }), hours)
}
