...
基于 [design.md](design.md) 的完全本地原生 Android 应用。已完成第一阶段基础框架，待办 CRUD、分类交互、番茄钟及正向计时。
...
- `AppLifecycleObserver` 和 lifecycle-process 依赖已预留，尚未注册分心业务监听。

已支持创建、编辑、完成/恢复、确认删除待办，选择默认或自定义分类，并按分类筛选。标题不能为空，目标时长为 1–1440 分钟，编辑草稿支持 Activity 重建恢复。

番茄钟支持关联待办或自由专注、自定义专注/休息时长、暂停继续、提前结束、休息倒计时及本轮摘要。时间锚点持久化到 Room，数据库已升级为版本 2 并提供 1→2 迁移。

正向计时从零开始，不设计划截止时间，支持暂停、恢复与结束保存，不自动进入休息。

当前尚未实现分心检测、统计图表及设置编辑。应用不申请网络权限，不包含账号、服务器或云同步；关闭 Android 自动备份。

下一阶段按设计书开发 ProcessLifecycleOwner 前后台检测与分心记录。重复待办和提醒仍留待后续阶段。
