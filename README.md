...
基于 [design.md](design.md) 的完全本地原生 Android 应用。已完成第一阶段基础框架，待办 CRUD、分类交互、番茄钟、正向计时及前后台分心检测。
...
番茄钟支持关联待办或自由专注、自定义专注/休息时长、暂停继续、提前结束、休息倒计时及本轮摘要。时间锚点持久化到 Room，数据库已升级为版本 3，提供 1→2→3 迁移。
...
当前尚未实现完整单次专注报告、统计图表及完整设置编辑。应用不申请网络权限，不包含账号、服务器或云同步；关闭 Android 自动备份。

下一阶段按设计书开发单次专注报告，再开发日/周/月统计。重复待办和提醒仍留待后续阶段。
