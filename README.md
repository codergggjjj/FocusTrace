...
基于 [design.md](design.md) 的完全本地原生 Android 应用。目前只实现设计书第 64 节的第一阶段。
...
当前尚未实现待办编辑、计时、分心检测、统计图表及设置编辑。应用不申请网络权限，不包含账号、服务器或云同步；关闭 Android 自动备份。

下一阶段按设计书实现待办 CRUD 和分类交互，再开发番茄钟、正向计时、分心检测。
