...
基于 [design.md](design.md) 的完全本地原生 Android 应用。已完成第一阶段基础框架，待办 CRUD、分类交互及番茄钟。
...
当前尚未实现正向计时、分心检测、统计图表及设置编辑。应用不申请网络权限，不包含账号、服务器或云同步；关闭 Android 自动备份。

下一阶段按设计书开发正向计时，再开发分心检测。重复待办和提醒仍留待后续阶段。
