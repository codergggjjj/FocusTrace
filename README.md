...
番茄钟支持关联待办或自由专注、自定义专注/休息时长、暂停继续、提前结束、休息倒计时及本轮摘要。时间锚点持久化到 Room，数据库已升级为版本 4，提供 1→2→3→4 迁移。
...
当前尚未实现统计图表及完整设置编辑。应用不申请网络权限，不包含账号、服务器或云同步；关闭 Android 自动备份。

下一阶段按设计书开发日/周/月统计。重复待办和提醒仍留待后续阶段。
